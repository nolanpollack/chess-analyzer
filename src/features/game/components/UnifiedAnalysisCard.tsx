...
export function UnifiedAnalysisCard({
  ...
  return (
    <div
      className={`flex h-190 flex-col overflow-hidden rounded-lg border bg-surface ${flagged ? "border-border-strong" : "border-divider"
        }`}
    >
      <AnalysisSection
        move={move}
...
function AnalysisHeader({ move, flagged }: AnalysisHeaderProps) {
  ...
  return (
    <div
      className={`flex flex-col gap-1 px-5 py-3.5 ${flagged ? "bg-tint-blunder" : ""
        }`}
    >
...
function BestMoveStack({
  ...
  return (
    <div>
      <EyebrowLabel>Best move</EyebrowLabel>
...
function EvalDelta({ evalBefore, evalAfter, side, className }: EvalProps) {
  ...
  let label: string;
  if (isMateScore(Math.abs(after))) {
    label = formatEval(Math.abs(after));
  } else {
  ...
  return (
    <span
      className={`inline-flex items-center gap-0.5 rounded-xs px-1.5 py-0.5 ${isUp ? "bg-surface-2 text-fg-2" : "bg-tint-blunder text-blunder"
        } ${className ?? ""}`}
    >
      {isUp ? (
